...
//v.2.0.4
        ...
        float4 fragMainFactor()
        {

        }                         

        float3  calcAdditinalDoubleShadeFeather(UtsLight additionalLight, VertexOutput i, float3 normalDirection, float3 viewDirection, float4 _MainTex_var)
        {
            float2 Set_UV0 = i.uv0;
            float notDirectional = 1.0f; //_WorldSpaceLightPos0.w of the legacy code.
            half3 additionalLightColor = GetLightColor(additionalLight);
...
////// Lighting:
                ...
                for (int iLight = -1; iLight < pixelLightCount; ++iLight)
				{
                    if (iLight != mainLightIndex)
                    {
                        ...
                        float3 finalColor = calcAdditinalDoubleShadeFeather(additionalLight, i, normalDirection, viewDirection, _MainTex_var);
                        pointLightColor += finalColor;
